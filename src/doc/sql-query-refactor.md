...
| 路径 | SELECT 预算 | 数据量关系 |
| --- | ---: | --- |
| 流水分页 | 3 | count、page、当前页摘要与生效状态 batch；1/20 行查询数一致，不读取原始载荷 |
...
2026-09-17 详情工作区扩展：文件详情增加两条聚合 SELECT（有效关系计数与按币种/方向金额），使用事实 ID 半连接避免重复来源行倍增。账本列表新增展示摘要和有效性字段，仍保持一条记录对应一个账本对象；相关回归见 `test_ledger_display_context_has_bounded_query_count`。字段与界面口径见 `inspection-workspace.md`。
