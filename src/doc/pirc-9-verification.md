...
- 代码：LedgerEntry 只发布 INCOME_AND_EXPENSE、INTERNAL_TRANSFER、ASSET_AND_LIABILITY；旧列、旧读取 API 与 `ledger_entry_source` 已删除。
...
- 账本：一个 Fact 可拆成多条 LedgerEntry，但一条 LedgerEntry 只对应一个 Fact；严格守恒并支持默认 INCOME_AND_EXPENSE 回补。ASSET_AND_LIABILITY 当前仅分类现金流水，不维护资产负债余额。
...
启动时自动升级为 10 表结构，并为尚无确认覆盖的历史 Fact 建立 DEFAULT Review 与等额 INCOME_AND_EXPENSE。旧 Ledger 行先收敛为 11 个字段，来源追溯统一改由 Allocation 完成。
