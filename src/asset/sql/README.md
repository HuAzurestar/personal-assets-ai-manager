...
- Times are ISO-8601 text with exactly six fractional digits. Generated audit
  timestamps use UTC with a `Z` suffix; source occurrence times must be
  normalized before persistence. SQLite defaults zero-fill digits unavailable
  from its clock, while application writes preserve all available microseconds.
  Database initialization pads legacy millisecond values to six fractional
  digits so text ordering and optimistic-lock equality remain valid.
