...
CREATE TABLE IF NOT EXISTS review_allocation /* Review、Transaction Fact 与 Ledger Entry 的已发布三元金额关系 */ (
    ...
    amount INTEGER NOT NULL DEFAULT 0 /* 按 currency_code 对应最小单位表示的本次分配正整数金额 */ CHECK (amount > 0),
    currency_code TEXT NOT NULL DEFAULT '' /* 币种与精度单位；必须与 Fact 和 Ledger Entry 完全一致 */ CHECK (currency_code <> ''),
