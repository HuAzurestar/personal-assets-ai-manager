...
function calendarMarkup(summary, currency, range) {
  ...
  const cells = Array.from({ length: range.last }, (_, index) => {
    ...
    return `<button type="button" class="calendar-day${item ? "" : " empty"}" data-action="account-day" data-value="${key}">
      <span>${day}</span>
      ${item ? `<strong>＋${amount(item.income_value, currency)}</strong><small>−${amount(item.expense_value, currency)}</small><em>${item.net_value >= 0 ? "+" : ""}${amount(item.net_value, currency)}</em>` : ""}
    </button>`;
  });
...
export function accountsMarkup({ summary, accounts, accountCode, currency, cursor }) {
  ...
  const activityCards = visibleActivities
    .map((item) => `<button type="button" class="activity-card" data-action="account-type" data-value="${esc(item.entry_type_code)}">
      ...
      <div><p><span>流入</span><strong>${amount(item.in_amount_value, selectedCurrency)}</strong></p><p><span>流出</span><strong>${amount(item.out_amount_value, selectedCurrency)}</strong></p></div>
  ...
  return `<section class="account-dashboard">
    ...
    <div class="month-metrics">
      <button type="button" data-action="account-metric" data-value="INCOME"><span>本月收入</span><strong class="income">${amount(total.income_value, selectedCurrency)}</strong><small>点击查看收入流水</small></button>
      <button type="button" data-action="account-metric" data-value="EXPENSE"><span>本月支出</span><strong class="expense">${amount(total.expense_value, selectedCurrency)}</strong><small>点击查看支出流水</small></button>
      <article><span>本月净收支</span><strong>${amount(total.net_value, selectedCurrency)}</strong><small>收入减支出并计入退款抵扣</small></article>
