...
原因：[target-ledger.js](../app/static/target-ledger.js) 的 `toast()` 把提示追加到 `document.body`，而 `modal()` 使用原生 `showModal()` 顶层弹窗；[ledger.css](../app/static/ledger.css) 第 520–522 行对 backdrop 应用模糊。普通 z-index 不能让 body 中的提示超越原生模态顶层。

建议：把表单失败原因直接显示在当前弹窗内，保留输入并引导用户重试/刷新；全局提示需要采用与当前模态层兼容的展示方式。

证据：

- [第一轮 JUnit 原始结果](docker-evidence/results.xml)。
...
- [镜像定义](Dockerfile.pirc9)；[构建上下文白名单](Dockerfile.pirc9.dockerignore)。镜像只复制 requirements 和 app，不包含本地数据库、虚拟环境或真实账单。
...
```powershell
docker build -t paam:pirc9-review-5d051bc -f reports/Dockerfile.pirc9 .
...
./.venv/Scripts/python.exe -m pytest reports/test_pirc9_docker.py -q --tb=short --junitxml=reports/docker-evidence/results.xml
```
...
- 静态/结构检查：`node --check`、`compileall`、`git diff --check` 和 `scripts/verify_target_ui.py` 均通过。
- 针对性回归：**11 passed**，46.89 秒。
- 全新 Docker HTTP/真实无头 Edge 验收：**15 passed**，59.97 秒。
- 最终 JUnit 证据：[final-accepted-results.xml](docker-evidence/final-accepted-results.xml)。
