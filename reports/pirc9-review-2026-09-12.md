...
- 新增独立检查文件：[test_pirc9_regressions.py](test_pirc9_regressions.py)。这些是以正确行为为断言的缺陷复现测试，当前失败是检查结果，不代表已修复；不在默认 `testpaths = tests` 中。
- 规则依据：仓库 fact-layer、review-layer、ledger-api 模块技能与 `docs/data-model.md`；特别核对不可变事实、显式分配、原子投影和并发版本。
...
- 原因：[target_review_projection_service.py](../app/services/target_review_projection_service.py) `publish()` 第 35–43 行直接把完整 Fact 传给 `_leg()`，金额计算未使用 Review Line 的分配。
...
- 原因：[target_ledger_summary_service.py](../app/services/target_ledger_summary_service.py) 第 38–43 行先按类型、币种聚合，再在 `_business_totals()` 第 93 行计算正负净额。日期分组同样不能保证逐事项口径。
...
- 原因：[target-ledger.js](../app/static/target-ledger.js) `render()` 第 117–139 行在 await 后无请求序号/路由一致性校验，成功及错误响应均可能覆盖新页面。
...
- 原因：[target-ledger.js](../app/static/target-ledger.js) `transitionReview()` 第 402–404 行只关闭最近的 dialog，`render()` 仅刷新底层页面。账户和冲突 transition 采用同样模式。
...
- 后端：[target_review_service.py](../app/services/target_review_service.py) 第 203 行明确允许 PENDING、REVOKED 的 update。
- 前端：[target-ledger.js](../app/static/target-ledger.js) 第 277、295–298 行阻断了后端支持的“撤销后修订”流程，用户只能恢复旧决定或另建事项。
...
- 原因：[target-ledger.js](../app/static/target-ledger.js) 第 170 行把不可变 Fact 的 `account_code` 当作编辑默认值。展示“原始账户”本身合理，但编辑应使用该 Fact 的有效 ACCOUNT Review。
...
- 原因：[target-ledger.js](../app/static/target-ledger.js) `importPage()` 第 193–197 行只返回空的 `#import-preview`；渲染完成后未调用 `renderImportPlan()`。
...
- 原因：[target-ledger.js](../app/static/target-ledger.js) `submitReview()` 第 386–392 行无提交中状态，且每次提交重新生成幂等键。
...
- 原因：[target_tag_projection_service.py](../app/services/target_tag_projection_service.py) 第 52–56 行发现合并流水的历史标签不一致并抛 ValueError；[target_tag_service.py](../app/services/target_tag_service.py) `set_view_status()` 没有转为领域错误，而 Controller 只捕获 TargetTagError。
...
- 原因：[target-ledger.js](../app/static/target-ledger.js) 第 264 行固定 `limit=200`；[target_review_mapper.py](../app/mappers/target_review_mapper.py) `list()` 按 ID 倒序截断；[target_review.py](../app/api/controllers/target_review.py) 第 106–113 行无游标/offset 参数。
...
./.venv/Scripts/python.exe -m pytest reports/test_pirc9_regressions.py -q --tb=short
