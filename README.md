...
## 2026-09-11 统一导入（当前导入行为以本节为准）

导入改为“上传 → 自动预览 → 确认”，支持文件选择、拖放和粘贴文件。自动识别支付宝、微信、建行 XLS、农行 PDF 和招行 PDF；混合来源一起处理，账户可在预览纠正并记住匹配。不同字段的重复导出补充原始证据，不重复增加流水金额。ZIP 密码只输入一次。

本次 7 份真实样本共 827 条记录：形成 803 笔流水、17 条补充证据，另保留 7 条已关闭记录。已验证合批、逐份、逆序及重复确认。原始记录可在导入弹窗的“最近导入记录”查看。

完整数据结构、兼容迁移、运行命令和支持边界见 [统一导入说明](docs/unified-import.md)。更新依赖后运行；独立测试环境入口为 `python scripts/run_import_test.py`，监听 8765，使用 `data/import-test/ledger.db`。
...
- 规则和模型只留下建议；人工确认、标签撤销后再修改正常生效。
- 退款性质独立保留，未分配退款不算收入；概览分别展示个人收支、退款抵扣及实际现金变化，并支持金额贡献回钻。
- 确认的重复组不自动扩张；冲突候选不能同时生效，撤销按剩余有效决定重算。代收代付改由手工事项分配，停止新增整笔第三方排除。
- 导入问题行保存原始证据并显示告警，在“数据问题”核验、修正后入账；非实际人民币收付可记录依据并重新核验。
- 流水详情提供账户修订、标签确认/撤销和退款性质入口；复核页提供事项、退款和问题记录管理。

范围、旧数据兼容、报表口径及尚未支持的组合见 [复核基座说明](docs/review-foundation.md)。尤其是 AA 分摊与退款混合、多币种和跨事项抵债，当前会明确限制，不自动猜测。

验证：`python -m pytest -q`；真实浏览器验证：`python scripts/verify_ui.py`、`python scripts/verify_review_ui.py`（需 Playwright）。以下旧版介绍涉及置信度抢占、第三方整笔排除、退款收入及导入失败处理的内容，已由本节与复核基座说明取代。

## 2026-09 UI 重构（当前界面以本节为准）

工作台默认采用“04 深色专注”，保留概览、流水、标签、复核、数据库，并新增“设置 → 外观主题”。四种主题（清爽青绿、克制蓝灰、暖白纸感、深色专注）即时切换，偏好保存在当前浏览器；加载前应用主题以避免浅色闪屏。当前首页使用 `ledger.js`、`ledger.css` 及 `theme.js`、`themes.css`；旧工作台脚本作为兼容资源保留，不再用于首页。

- 流水优先展示，搜索常驻，日期、金额、账户、来源、标签与排序放在可展开的高级筛选中。导航与筛选写入 URL，浏览器前进/后退可恢复查询；翻页回到页面顶部。
- 导入是独立面板，支持文件/文件夹、CSV/XLS/XLSX/密码 ZIP，逐文件展示原始字段抽样。换文件或平台会使预览失效；错误与重复文件不会确认导入。ZIP 密码只用于当前请求，确认前需要重新输入。
- 标签页支持维度和标签的新建、重命名、归档、恢复。流水支持单条和批量标签编辑；批量修改只影响选中的维度，其他维度保留。保存失败保持弹窗与输入，可重试。
- 复核使用对照卡片，保留原始字段默认展开、人工决定、批量处理和撤销。状态使用中文，已确认转移筛选同时包含个人及他人转移；“稍后”可通过“继续处理”恢复为待处理。
- `GET /api/transactions` 新增 `scope=effective|all|excluded`，默认仍是参与汇总的流水；原始记录可随时找回。概览区分累计保存、已排除和所选日期内参与汇总的数量。
- `PUT /api/transactions/bulk-tag-state` 新增可选 `merge=true`，仅修改提交的标签维度；默认替换行为保持兼容。该操作仍在单个事务内完成。
- 数据库页优先展示行，结构和索引按需展开。统一事件处理和请求版本检查，避免重复翻页、旧请求覆盖新页面。

### 宽屏与连续操作优化

- 导航与主区域整体居中，超宽屏不再把导航留在远端；1100px 以下使用顶部导航，851–1100px 保留完整流水表格。
- 桌面流水表局部滚动、表头固定；低高度窗口压缩页头与筛选区。选中流水/候选后底部常驻批量栏，并可取消选择。
- 保存标签和处理候选保留页面及表格滚动位置，翻页仍回顶部；筛选修改后明确提示尚未应用。
- 确认转移、保留重复项以及批量复核先显示影响确认；取消不提交。单条复核提交后提供 20 秒快捷撤销，之后仍可在复核记录中撤销。
- 本次不改变账目统计口径；双栏详情、行内快速分类和批量标签历史回退尚未实现。

运行服务后打开 `/#settings` 切换主题；`/static/style-cards.html` 仍保留四张静态对比卡片。

验证：`python -m pytest -q`。真实浏览器回归需单独安装 `playwright`，Windows 使用已安装的 Edge：
...
脚本在临时目录启动隔离账本，不接触现有服务或真实数据库；浏览器截图与结果位于 `data/ui-verification/`。非 Windows 环境需先安装 Playwright Chromium。旧章节中四种打标策略仍是后端能力，当前 UI 入口为按名称选择标签，不再显示旧策略按钮。
